...
class RobotState:
    ...
    def load_urdf(self, urdf_file: str) -> None:
        try:
            ...
            for joint in root.findall("joint"):
                ...
                if initial_pos is not None and initial_pos.text is not None:
                    try:
    ...
    @staticmethod
    def quintic_interpolation(t: float, duration: float) -> float:
    ...
    def calculate_interpolated_positions(
        self,
        ...
        for joint in target_pos:
            new_positions[joint] = round(
                start_pos[joint]
                + (target_pos[joint] - start_pos[joint]) * interp_factor,
                2,
            )
    ...
    def execute_movement(
        ...
        target_pos: dict[str, float],
        duration: float,
        callback: Callable[[dict[str, float] | None, str | None], None] | None = None,
    ) -> None:
        """
